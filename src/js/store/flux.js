...
export const createActions = ({ setStore, getStore }) => ({
	fetchCohorts: () => {
			...
			.then(({ data: students }) => {
				...
				fetch(url, { cache: "no-cache" })
					...
					.then(activities => {
						...
						Object.keys(dailyAvg).map(key => (dailyAvg[key] = (dailyAvg[key] / students.length) * 100));
						// divide by the amount of days recorded to get the avg
						Object.keys(stuAct).map(
							key => (stuAct[key].avg = (stuAct[key].avg / (Object.keys(stuAct[key]).length - 1)) * 100) // Minus the avg key
						);
	...
	formatNames: data => {
		...
		for (let i in data) {
			...
			let username = data[i].username === undefined ? getUserName(data[i].email) : getUserName(data[i].username);
			// first_name: null
